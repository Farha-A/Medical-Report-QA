...
def load_colpali(model_id: str):
    ...
    model = ColPali.from_pretrained(model_id, torch_dtype=dtype).to(device).eval()
...
def colpali_rank(processor, model, image: Image.Image, top_k: int) -> list[tuple[str, float]]:
    img_inputs = processor.process_images([image]).to(model.device)
    q_inputs = processor.process_queries(CANDIDATE_FINDINGS).to(model.device)
    with torch.inference_mode():
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        colpali_id = top_k = use_groq = groq_model = None
        if backend.startswith("MedGemma"):
            ...
            max_new = st.slider("Max new tokens", 64, 1024, 384, step=32)
    ...
    if backend.startswith("MedGemma"):
        ...
        with st.spinner("Scoring findings..."):
            ranked = colpali_rank(processor, model, image, top_k)
        retrieved = format_retrieval(ranked)
        if use_groq:
            with st.spinner(f"Synthesizing with Groq ({groq_model})..."):
                try:
                    synth = synthesize_with_groq(ranked, groq_model)
                    report = f"{synth}\n\n---\nRetrieval debug:\n{retrieved}"
